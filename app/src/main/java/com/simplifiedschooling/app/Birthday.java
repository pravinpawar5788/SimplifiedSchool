...
import com.simplifiedschooling.app.adapter.BirthDayListAdapter;
...
public class Birthday extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mStudentList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			/*	String Student_Id = null;
				Student_Id = StudentId.get(arg2).toString();
				//String classDivId = classDivIdList.get(arg2).toString();
				Intent i = new Intent(AllStudentList.this,
						Student_details.class);

				i.putExtra("Student_Id", Student_Id);
				i.putExtra("ClassDivId", classdivId);
	...
	private class TeacherclassListener implements OnItemSelectedListener {
		...
		private void GetStudentDetailsList() {
			...
			StringRequest strReq = new StringRequest(Request.Method.GET,
						AppConfig.BASE_URL + "birthdayacts/" + classDivId,
					new Response.Listener<String>() {
						...
						public void onResponse(String response) {
							...
							try {
								...
								for (int k = 0; k < calssdetailsArray.length(); k++) {
									...
									String date = c1.getString("dob");
									String dates[] = date.split("T");
									String dateW = dates[0];
									String mdate[] = dateW.split("-");
									...
									/*
									 * Date dateW1 = new Date(dateW);
									 * SimpleDateFormat formatter = new
									 * SimpleDateFormat("EEE, MMM d"); String
									 * dateS = formatter.format(dateW1);
									 */
									movie.setStudentId(toDate);

									// adding movie to movies array
									studentInfo.add(movie);
									if (studentInfo.size() == 0) {
										// submit.setVisibility(View.INVISIBLE);
										Toast.makeText(Birthday.this,
												"There is no student!",
										...
										adapter = new BirthDayListAdapter(
												Birthday.this, studentInfo);
										mStudentList.setAdapter(adapter);
										adapter.notifyDataSetChanged();
									}
