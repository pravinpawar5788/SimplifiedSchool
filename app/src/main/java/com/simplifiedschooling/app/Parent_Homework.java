...
public class Parent_Homework extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myTabHost.setup();
		TabSpec spec = myTabHost.newTabSpec("Today's Homework");
		spec.setContent(R.id.tabTodayshomwork);
		spec.setIndicator("Today's Homework");
		myTabHost.addTab(spec);
		TabSpec spec1 = myTabHost.newTabSpec("Homework This week");
		spec1.setContent(R.id.tabWeekhomework);
		spec1.setIndicator("Homework This week");
	...
	protected void GetArichiveHomework(final int position) {
		...
		String tag_string_req = "Get Archive Homework ";
		...
		StringRequest strReq = new StringRequest(
				Request.Method.POST,
				AppConfig.BASE_URL+"weekwisemobdatas", new Response.Listener<String>() {
						...
						try {
							...
							for (int i = 0; i < data.length(); i++) {
								...
								if (Constant.homeworkdata.size() == 0) {
									...
									Toast.makeText(Parent_Homework.this,
											"There is no Data!",
									...
									homeworkAdapter = new CustomHomeworkAdapter(
											Parent_Homework.this,
											Constant.homeworkdata);
									ArchivehomeworkList.setAdapter(homeworkAdapter);
					...
					protected Map<String, String> getParams() {
						...
						params.put("weekno", weekNo.get(position).toString());
	...
	protected void Getweek() {
		...
		StringRequest strReq = new StringRequest(Request.Method.GET,
				AppConfig.BASE_URL+"weeks", new Response.Listener<String>() {
					...
					public void onResponse(String response) {
						...
						try {
							...
							for (int k = 0; k < calssdetailsArray.length(); k++) {
								...
								String fday=sdate[2];
								Date fFdate=new Date(Integer.valueOf(fyear),Integer.valueOf(fmonth)-1,Integer.valueOf(fday));
								...
								String tday=mdate[2];
								Date tFdate=new Date(Integer.valueOf(tyear),Integer.valueOf(tmonth)-1,Integer.valueOf(tday));
								...
								/*    Date fromDate=null;
								try {
									fromDate = (Date) formatter.parse(fdate);
						        ...
						        Date toDate=null;
								try {
									toDate = (Date) formatter.parse(tdate);
								} catch (ParseException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
								}*/
								weeks.add("Week No:"+weekno+" "+fromDate+"-"+toDate);
								weekNo.add(weekno);
                                if(weeks.size()>0)
                                {
                                	ArrayAdapter<String> studentlistadapter = new ArrayAdapter<String>(
											Parent_Homework.this, android.R.layout.simple_spinner_item,weeks);
                                	weekList.setAdapter(studentlistadapter);
                                }else{
                                	
                                	Toast.makeText(getBaseContext(), "There is no week!",
	...
	protected void GetWeekHomework() {
		...
		StringRequest strReq = new StringRequest(
				Request.Method.GET,
				AppConfig.BASE_URL+"parentcurrentweeks/"
						+ User_id, new Response.Listener<String>() {
							...
							for (int i = 0; i < data.length(); i++) {
								...
								if (Constant.homeworkdata.size() == 0) {
									...
									homeworkAdapter = new CustomHomeworkAdapter(
											Parent_Homework.this,
											Constant.homeworkdata);
									weekhomeworkList.setAdapter(homeworkAdapter);
									homeworkAdapter.notifyDataSetChanged();
								}
	...
	private void GetTodaysHomework() {
		...
		StringRequest strReq = new StringRequest(
				Request.Method.GET,
				AppConfig.BASE_URL+"parenthomeworks/"
						+ User_id, new Response.Listener<String>() {
